...
/**
 * Create a new user.
 * @param {{name?:string,email:string,password_hash:string,role_id?:number,status?:string,email_verified?:boolean,verification_token?:string,verification_token_expiry?:Date,device_id:string}} user
 * @returns {Promise<object>} inserted user row
 */
export async function createUser(user) {
	const text = `INSERT INTO "User" (name, email, password_hash, role_id, status, email_verified, verification_token, verification_token_expiry, device_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`;
	const values = [
		user.name || null,
		user.email,
		user.password_hash,
